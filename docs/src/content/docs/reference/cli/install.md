...
| `--only apm\|mcp` | both | Install only APM packages or MCP/LSP service dependencies. Use `--only=apm` to hide service config and `--only=mcp` to preview MCP and LSP services only. |
